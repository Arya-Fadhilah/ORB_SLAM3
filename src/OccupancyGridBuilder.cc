...
OccupancyGridBuilder::OccupancyGridBuilder(Map* pMap, const std::string &strSettingsPath, Settings* settings)
    : mpMap(pMap), mbFinishRequested(false) {

    if (settings) {
        // Format baru — pinjam dari Settings yang sudah diparsing System, tanpa baca file lagi
        GeometricCamera* cam1 = settings->camera1();

        if (cam1 && cam1->size() >= 4) {
            mFx = cam1->getParameter(0);
            mFy = cam1->getParameter(1);
            mCx = cam1->getParameter(2);
            mCy = cam1->getParameter(3);

            cv::Size imSize = settings->newImSize();
            mImageWidth  = imSize.width;
            mImageHeight = imSize.height;

            mbCalibrationLoaded = (mImageWidth > 0 && mImageHeight > 0 && mFx > 0.f && mFy > 0.f);
        } else {
            mbCalibrationLoaded = false;
        }

        if (!mbCalibrationLoaded) {
            std::cerr << "[OccupancyGridBuilder] FATAL: objek Settings ada tapi kalibrasi kamera "
                      << "tidak valid (cam1 null atau parameter kurang dari 4) — occupancy grid "
                      << "TIDAK akan dibangun." << std::endl;
        }

    } else {
        // Format lama/KITTI — mandiri, baca & parsing file yaml sendiri
        mbCalibrationLoaded = LoadCameraIntrinsics(strSettingsPath);
        if (!mbCalibrationLoaded) {
            std::cerr << "[OccupancyGridBuilder] FATAL: gagal memuat kalibrasi kamera dari "
                      << strSettingsPath
                      << " — occupancy grid TIDAK akan dibangun (mencegah hasil silently-wrong)."
                      << std::endl;
        }
    }
...
cv::Mat OccupancyGridBuilder::GetOccupancyGrid() {
    std::lock_guard<std::mutex> lock(mGridMutex);
    return mGlobalOccupancyGrid.clone();   // clone() penting — salin di dalam lock, bukan referensi
}
...
bool OccupancyGridBuilder::LoadCameraIntrinsics(const std::string &strSettingsPath) {
    cv::FileStorage fSettings(strSettingsPath, cv::FileStorage::READ);
    if (!fSettings.isOpened()) {
        std::cerr << "[OccupancyGridBuilder] Tidak bisa membuka settings file: "
                  << strSettingsPath << std::endl;
        return false;
    }

    // Deteksi format yaml — sama seperti logika di System::System()
    cv::FileNode versionNode = fSettings["File.version"];
    bool bNewFormat = (!versionNode.empty() && versionNode.isString()
                        && versionNode.string() == "1.0");

    // Format baru pakai prefix "Camera1.", format lama pakai "Camera."
    const std::string prefix = bNewFormat ? "Camera1." : "Camera.";

    cv::FileNode fxNode = fSettings[prefix + "fx"];
    cv::FileNode fyNode = fSettings[prefix + "fy"];
    cv::FileNode cxNode = fSettings[prefix + "cx"];
    cv::FileNode cyNode = fSettings[prefix + "cy"];
    cv::FileNode wNode  = fSettings["Camera.width"];
    cv::FileNode hNode  = fSettings["Camera.height"];

    if (fxNode.empty() || fyNode.empty() || cxNode.empty() || cyNode.empty()
        || wNode.empty() || hNode.empty()) {
        std::cerr << "[OccupancyGridBuilder] Parameter kalibrasi tidak lengkap di "
                  << strSettingsPath << " (dicari dengan prefix '" << prefix << "')."
                  << " Cek apakah nama key di yaml-mu cocok." << std::endl;
        return false;
    }

    mFx = (float) fxNode.real();
    mFy = (float) fyNode.real();
    mCx = (float) cxNode.real();
    mCy = (float) cyNode.real();
    mImageWidth  = (int) wNode.real();
    mImageHeight = (int) hNode.real();

    std::cout << "[OccupancyGridBuilder] Kalibrasi dimuat dari " << strSettingsPath
              << " (format " << (bNewFormat ? "baru/1.0" : "lama") << "): "
              << "fx=" << mFx << " fy=" << mFy
              << " cx=" << mCx << " cy=" << mCy
              << " size=" << mImageWidth << "x" << mImageHeight << std::endl;
...
void OccupancyGridBuilder::BuildOccupancyGrid() {
    if (!mpMap) return;
    if (!mbCalibrationLoaded) return;   // guard: jangan bangun grid dengan kalibrasi kosong/salah
    ...
    // Diganti: baca dari member yang dimuat lewat LoadCameraIntrinsics(), bukan hardcoded
    float fx = mFx;
    float fy = mFy;
    float cx_intrinsics = mCx;
    float cy_intrinsics = mCy;
